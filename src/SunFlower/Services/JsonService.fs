...
module JsonService =
    ...
    let instance = { FilePath = ""; Data = None }
    /// <summary>
    /// Remember the file
    /// </summary>
    /// <param name="path"></param>
    /// <param name="state"></param>
    let touch path state = { state with FilePath = path }
    ...
    let load<'T> (name: string) : 'T list =
        let path =
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Registry", name + ".json")
    ...
    /// </summary>
    /// <param name="name">file name without extension</param>
    let loadAsync<'T> (name: string) : Task<'T list> =
        task {
            let path =
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Registry", name + ".json")
    ...
    /// <param name="name">file name without extension</param>
    /// <param name="data">list of objects</param>
    let saveAsync<'T> (name: string) (data: 'T list) =
        task {
            let path =
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Registry", name + ".json")
