...
type RecentViewModel() =
    ...
    member this.RemoveItemAsync(item: FlowerFileInfo) =
        task {
            match this.Recent.Contains(item) with
            | false -> return ()
            | true ->
                this.Recent.Remove(item) |> ignore
                do! this.Recent |> Seq.toList |> JsonService.saveAsync "recent"
        }
    ...
    /// </summary>
    /// <param name="item">record with target item</param>
    member this.RemoveFileAsync(item: FlowerFileInfo) =
        task {
