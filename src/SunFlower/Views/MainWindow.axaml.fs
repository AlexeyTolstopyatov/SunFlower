...
type MainWindow() as this =
    ...
    member this.ImportAsync() =
        task {
            ...
            | _ ->
                ...
                do! fileInfos @ newFileInfos |> JsonService.saveAsync "recent"
                // Make sure that data context bound correct
                match this.DataContext with
                | :? MainWindowViewModel as ctx -> do! ctx.UpdateRecentContext()
                | anotherCtx ->
#if DEBUG
                    $"Incorrect binding: \"{anotherCtx.GetType()}\", expected \"MainWindowViewModel\" "
    ...
    member private this.OpenDialog(_: obj, _: RoutedEventArgs) =
        ...
        task {
            ...
            with e ->
                "Avalonia can't open dialog" |> Console.Error.WriteLine
#endif
        }
        |> Async.AwaitTask
        |> Async.Start
