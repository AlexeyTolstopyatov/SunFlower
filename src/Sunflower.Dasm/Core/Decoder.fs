...
type FlowControl =
    ...
    | IndirectJumpFar
    | IndirectCallFar
    | SoftwareInterrupt // остаётся для INTO и нераспознанных INT
...
module internal Decoder =
    ...
    let private classifyFlow (state: DecoderState) (offset: int) (length: int) (bytes: byte[]) : FlowControl * int list * string option =
        ...
        | 0xEBuy -> UnconditionalJump, rel8Target offset length bytes, None
        | 0xE9uy -> UnconditionalJump, rel16Target offset length bytes, None
